...
`POST /api/evidence` ponownie waliduje claim i wykonuje jeden backendowy Tavily Search z parametrami `basic`, `general`, maksymalnie 5 wyników oraz bez answer, raw content i obrazów. Wyniki są normalizowane do `url`, `title`, `content` i technicznego `retrievalScore`. Dla niepustej listy frontend wysyła claim i candidates do osobnego `POST /api/evidence/analyze`. Evidence Analyst otrzymuje wyłącznie claim oraz `candidateIndex` i `content`, po czym zwraca niezależnie walidowane `relation` i krótkie `reason` dla każdego materiału. Klasyfikacje są mapowane do candidates po `candidateIndex`, a frontend wysyła do Synthesizera wyłącznie `claim` oraz zbudowane na potrzeby requestu `{ content, relation, reason }`. UI pokazuje tytuł, link, fragment, relację i uzasadnienie.

Backendowy `POST /api/evidence/synthesize` przyjmuje zaakceptowany claim oraz 0–5 elementów `{ content, relation, reason }`. `overallPattern` jest wyliczany deterministycznie z istniejących relacji, a Groq generuje wyłącznie krótkie polskie `summary`, walidowane niezależnie do 500 znaków. Frontend uruchamia endpoint automatycznie po poprawnym Evidence Analysis, a dla `candidates: []` pomija Analysis i wysyła pustą listę analyzed evidence. Wynik jest pokazywany pod listą evidence jako „Łączny obraz dowodów”. Błąd Synthesizera nie usuwa poprawnie przeanalizowanych materiałów i nie uruchamia frontendowego retry.
